...
- **配额**：登录用户每项 20 次，未登录用户（匿名）每项 10 次；可通过充值三档套餐增加次数
- **个人资料**：昵称、头像（Emoji 或自定义图片）、修改密码
- **管理后台**：管理员访问 `/#admin` 或 `/admin`，含数据概览（含近1h/24h统计）、访问/使用记录、用户管理（增删改查、配额调整）、支付记录、成本监控等
...
│   ├── gunicorn_config.py    # Gunicorn 配置（日志过滤、超时设置）
│   ├── .env                  # 环境变量配置（需自行创建）
...
| 管理 | `GET /api/admin/stats` | 数据概览（用户数、支付、收入、近1h/24h统计、趋势） |
...
- **异步地理位置查询**：访问记录和使用记录的地理位置信息异步更新，不阻塞主要功能
- **日志优化**：生产环境自动过滤静态资源请求日志，只保留关键 API 请求和错误日志
- **按需加载**：管理后台数据只在用户点击对应标签页时才加载，减少不必要的数据库查询
- **文件上传优化**：大文件（>10MB）检测时只做快速元数据检测，避免超时；前端添加30秒超时保护
- **进度条优化**：加密/解密进度条平滑更新，避免卡在固定百分比，提升用户体验
...
- **日志输出**：生产环境已优化日志输出，只记录关键 API 请求和错误信息，减少日志噪音
- **数据加载**：管理后台采用按需加载策略，只在用户点击对应功能时才请求数据，提升性能
